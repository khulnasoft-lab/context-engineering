...
- A CLI application to interact with the Research Agent.
- A Research Agent capable of performing web searches using the Brave Search API.
- An Email Draft Agent that can create email drafts via the Gmail API.
- The Research Agent must be able to invoke the Email Draft Agent as a tool.
...
-   **Virtual Environment**: Ensure all Python commands and tests are executed within `venv_linux`.
-   **Project Awareness**: Always read `PLANNING.md` (if it exists) at the start of a new conversation and check `TASK.md` before starting a new task.
